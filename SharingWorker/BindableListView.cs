...
{
    /// <summary>
    /// Extends ListView by making the SelectedItems property read/write.
    /// ListView does have a SelectedItems property but it is read-only. 
    /// MultiSelectListView hides the base class's SelectedItems property.
    /// The SelectedItems property of MultiSelectListView can participate in two way data binding 
    ...
    public class BindableListView : ListView
    {
        ...
        {
            // Add handler for when the listView internal selection changes.
        ...
        public static new readonly DependencyProperty SelectedItemsProperty =
           DependencyProperty.Register("SelectedItems", typeof(IList), typeof(BindableListView),
           new FrameworkPropertyMetadata(
                  (d, e) =>
                  {
                      // When the property changes, update the selected values in the selectedItems box.
                      (d as BindableListView).SetSelectedItemsNew(e.NewValue as IList);
                  }));
        /// <summary>
        ...
        /// Because this class hides ListView.SelectedItems, it is necessary add the value directly
        ...
        /// the listView. NOTE: For the selectedItems box display to update, ItemsSource must impliment INotifyCollectionChanged

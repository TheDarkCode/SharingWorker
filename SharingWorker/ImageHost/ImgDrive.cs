    ...
    class ImgDrive : PropertyChangedBase, IImageHost
    {
        ...
        public async Task LogIn()
        {
            ...
            using (var client = new HttpClient(handler))
            {
                ...
                client.DefaultRequestHeaders.Referrer = new Uri("http://imgdrive.co/login.html");

                var content = new FormUrlEncodedContent(new[]
                {
                    new KeyValuePair<string, string>("op", "login"),
                    new KeyValuePair<string, string>("redirect", "http://imgdrive.co/"),
                    new KeyValuePair<string, string>("login", User),
                    new KeyValuePair<string, string>("password", Password)
                });

                using (var response = await client.PostAsync("http://imgdrive.co/login.html", content))
                {
                ...
                token = null;
                using (var response = await client.GetAsync("http://imgdrive.co/?op=my_files"))
                {
                    var result = response.Content.ReadAsStringAsync().Result;
                ...
                using (var client = new HttpClient(handler))
                {
                    ...
                    foreach (var image in uploadImages)
                    {
                        ...
                        using (var content = new MultipartFormDataContent(String.Format("----------{0}", boundary)))
                        {
                            ...
                            using (var message = await client.PostAsync("http://imgdrive.co/cgi-bin/upload_flash.cgi", content))
                            {
        ...
        public static async Task<string> GetImagesCode(string id)
        {
            ...
            using (var client = new HttpClient(handler))
            {
                client.DefaultRequestHeaders.Add("User-Agent", "Mozilla/5.0 (Windows NT 6.1; WOW64; rv:38.0) Gecko/20100101 Firefox/38.0");
                client.DefaultRequestHeaders.ExpectContinue = false;
                client.DefaultRequestHeaders.Referrer = new Uri("http://imgdrive.co/?op=my_files");
                client.DefaultRequestHeaders.Add("Connection", "Keep-Alive");
                ...
                using (var response = await client.PostAsync("http://imgdrive.co/", content))
                {
                    var result = response.Content.ReadAsStringAsync().Result;

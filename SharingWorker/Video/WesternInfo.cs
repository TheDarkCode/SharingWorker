...
namespace SharingWorker.Video
{
    static class WesternInfo
    {
        ...
        public static async Task<VideoInfo> GetDarkX(string id)
        {
            var url = string.Empty;
            var ret = new VideoInfo { Title = "", Actresses = "" };
            var num = id.Replace("DX_", string.Empty);

            url = string.Format("http://www.darkx.com/en/search/{0}?query={0}", num);
            using (var handler = new HttpClientHandler())
            using (var client = new HttpClient(handler))
            {
                var responseString = await client.GetStringAsync(url);
                ...
                if (start >= 0)
                {
                    ...
                    if (end >= 0)
                    {
                        ...
                        ret.Title = string.Format("[DarkX] {0} - {1}", title, ret.Actresses);
                    }
                }
            }
            return ret;
        ...
        public static async Task<VideoInfo> GetDDFNetwork(string id)
        {
            var url = string.Empty;
            var ret = new VideoInfo { Title = "", Actresses = "" };
            var num = id.Replace("DDF_", string.Empty);

            url = string.Format("https://ddfnetwork.com/scenes/view/{0}", num);
            using (var handler = new HttpClientHandler())
            using (var client = new HttpClient(handler))
            {
                var responseString = await client.GetStringAsync(url);
        ...
        public static async Task<VideoInfo> GetBANG(string id)
        {
            var url = string.Empty;
            var ret = new VideoInfo { Title = "", Actresses = "" };
            var num = id.Replace("BANG_", string.Empty);

            url = string.Format("https://5c5366a3b84ecedad405b95dbadea764.us-east-1.aws.found.io/videos/video/{0}", num);
            using (var handler = new HttpClientHandler
            {
